...
export default function RegisterScreen() {
  ...
  const sendCodeViaSMS = async (phone: string, codigo: string) => {
    console.log(`📱 Código ${codigo} enviado a ${phone}`);
    return true;
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // 6. Enviar código según método
      if (verificationMethod === 'email') {
        await sendCodeViaEmail(email, codigo);
      } else {
        await sendCodeViaSMS(city.code + phone, codigo);
      }

      // 7. Ir a pantalla de verificación
      router.push({
